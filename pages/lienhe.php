...
if ((isset($_SESSION['tenkhachhang'])) || (isset($_SESSION['tencongty']))) {
    if ((isset($_SESSION['tenkhachhang']))) {
        ...
        if ($ketqua) {
            foreach ($ketqua as $cty) {
                $id = $cty['id_dangky'];
                $sql = "SELECT * from tbl_dangky_nguoitimviec where id_dangky=?";
    ...
    if ((isset($_SESSION['tencongty']))) {
        $sql = "SELECT * from tbl_dangky_nhatuyendung";
        $ketqua = $conn->prepare($sql);
        $ketqua->execute();
        $ketqua = $ketqua->fetchAll(PDO::FETCH_ASSOC);
        if ($ketqua) {
            foreach ($ketqua as $cty) {
                $id = $cty['id'];
                $sql = "SELECT * from tbl_dangky_nhatuyendung where id=?";
                $ketqua1 = $conn->prepare($sql);
                $ketqua1->execute([$id]);
                $ketqua1 = $ketqua1->fetchAll(PDO::FETCH_ASSOC);
            }
    ...
    if (isset($_POST['dangky'])) {
        ...
        $sql = "INSERT INTO tbl_phanhoikhachhang(ten,email,nhanxet) VALUES(?,?,?)";
        $ketqua = $conn->prepare($sql);
        ...
        showConfirmButton: true,
            ...
            header("refresh:2;url=lienhe.php");
            exit;
        }
    }
